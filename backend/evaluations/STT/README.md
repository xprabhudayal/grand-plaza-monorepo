...
Copy `config_sample.json` to `config.json` and update with your API keys:
```json
{
  "providers": {
    "deepgram": {
...
3. Add your audio files to `Audio/` directory following the naming convention in ground_truth.py
...
✅ **All providers now use REAL SDKs (no more placeholders!)**
- **Deepgram**: Using `deepgram-sdk` with PrerecordedOptions
- **AssemblyAI**: Using `assemblyai` SDK with TranscriptionConfig  
- **Soniox**: Using direct REST API calls with requests
- **Groq Whisper**: Using `groq` SDK for Whisper models
...
- All providers are implemented with real SDKs - ready for production use
